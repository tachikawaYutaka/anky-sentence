...
public class ExaminationServiceTest {
    ...
    public void calculateConcordance(){
        ...
        ExaminationInput examinationInput = new ExaminationInput("hogehogehoge");
        Examination examination = new Examination(question.getQuestionId(),examinationInput);

        AnswerResult answerResult = examinationService.calculateConcordance(examination);
        log.info("info:" +  answerResult.getAnswerResultScore().getValue());
        Assert.assertTrue(answerResult.getAnswerResultScore().getValue() == 100);
    }

    @Test
    public void calculateConcordance2(){
        UserMailAddress userMailAddress = new UserMailAddress("hogehoge@hogehoge.com");
        UserPassword userPassword = new UserPassword("hogehoge",bCryptPasswordEncoder);
        User user = UserFactory.create(userMailAddress, userPassword);
        userService.save(user);
        ...
        ExaminationInput examinationInput = new ExaminationInput("hoge");
        Examination examination = new Examination(question.getQuestionId(),examinationInput);
